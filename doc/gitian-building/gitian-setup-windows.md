...
The build script we'll be using is contained in the SOVRANOCOIN github repository ([contrib/gitian-build.py](https://github.com/sovranocoin-project/sovranocoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the SOVRANOCOIN repository and will need to fetch this script with the following commands:
...
- Clone the gitian-builder, gitian.sigs, sovranocoin-detached-sigs, and sovranocoin GitHub repos.
