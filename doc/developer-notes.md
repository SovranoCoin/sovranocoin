...
- [Development guidelines](#development-guidelines)
    - [General SOVRANOCOIN Core](#general-sovranocoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SOVRANOCOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play SVRs (tSVR)" on the test network, if you
...
SOVRANOCOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/sovranocoin-project/sovranocoin/blob/master/contrib/valgrind.supp))
...
- BitcoinMiner : Generates SVRs (if wallet is enabled).
...
However, in open source software such as SOVRANOCOIN Core, where everyone uses
...
pay attention to for reviewers of SOVRANOCOIN Core code.

General SOVRANOCOIN Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: SOVRANOCOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against SOVRANOCOIN Core so that they can be integrated
...
file descriptors it will cause problems with SOVRANOCOIN's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `sovranocoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `sovranocoin-cli` and the GUI debug console use this table to determine how to
