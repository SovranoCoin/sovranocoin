...
from test_framework.test_framework import SovranocoinTestFramework
...
class ReorgStakeTest(SovranocoinTestFramework):
    ...
    def check_money_supply(self, expected_svr, expected_zsvr):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected SVR and zSVR supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_svr))
            for denom in node['zSVRsupply']:
                assert_equal(node['zSVRsupply'][denom], DecimalAmt(expected_zsvr[denom]))
    ...
    def run_test(self):
        ...
        # Check SVR and zSVR supply at the beginning
        # ------------------------------------------
        # zSVR supply: 2 coins for each denomination
        expected_zsvr_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # SVR supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zsvr_supply)
        ...
        # Verify that SVR and zSVR supplies were properly updated after the spends and reorgs
        self.log.info("Check SVR and zSVR supply...")
        ...
        expected_zsvr_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zsvr_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zsvr_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zsvr_supply)
