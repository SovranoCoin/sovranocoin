...
class ZerocoinSpendTest(SovranocoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zsvr_bal, svr_bal):
            zsvr_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zsvr_bal)
            svr_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], svr_bal)
            return zsvr_bal, svr_bal
        ...
        zsvr_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zsvr_balance, 6666)
        ...
        zsvr_balance, balance = check_balances(denom_2, zsvr_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zsvr_balance, balance = check_balances(denom_3, zsvr_balance, balance)
        ...
        _, _ = check_balances(denom_1, zsvr_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
