...
#include "zsvr/zsvrmodule.h"
#include "zsvr/zsvrwallet.h"
#include "zsvr/zsvrtracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_SOVRANOCOIN = 61000, // inherited version..
...
enum ZerocoinSpendStatus {
    ZSVR_SPEND_OKAY = 0,                            // No error
    ZSVR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSVR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSVR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSVR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSVR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSVR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSVR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSVR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSVR_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZSVR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSVR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSVR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSVR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSVR_SPENT_USED_ZSVR = 14,                      // Coin has already been spend
    ZSVR_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZSVR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSVROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zPIV wallet
    CzSVRWallet* zwalletMain{nullptr};
    std::unique_ptr<CzSVRTracker> zsvrTracker{nullptr};
    void setZWallet(CzSVRWallet* zwallet);
    CzSVRWallet* getZWallet();
    ...
    // zPIV reset
    boost::signals2::signal<void()> NotifyzSVRReset;
