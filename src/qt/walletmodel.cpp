...
bool WalletModel::createZsvrSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZsvr(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        CZerocoinSpendReceipt &receipt)
{
...
static void NotifyzSVRReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSVRReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSVRReset.connect(boost::bind(NotifyzSVRReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSVRReset.disconnect(boost::bind(NotifyzSVRReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (address.IsStakingAddress())
        return error("Invalid SOVRANOCOIN address, cold staking address");
    CKeyID keyID;
...
{
    if (!address.IsValid())
        return error("Invalid SOVRANOCOIN address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from SOVRANOCOIN address");
...
{
    setMints.clear();
    setMints = pwalletMain->zsvrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
