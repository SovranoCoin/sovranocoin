...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SovranoCoin address (e.g. %1)").arg("Sd9gp7rBszFvg75fwzUi3hByPekBCS5te5"));
#endif
...
{
    // return if URI is not valid or is no SovranoCoin: URI
    if (!uri.isValid() || uri.scheme() != QString(URI_SVREME))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::SVR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert sovranocoin:// to sovranocoin:
    //
    //    Cannot handle this later, because sovranocoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if (uri.startsWith(URI_SVREME "://", Qt::CaseInsensitive)) {
        uri.replace(0, std::strlen(URI_SVREME) + 3, URI_SVREME ":");
    }
...
{
    QString ret = QString(URI_SVREME ":%1").arg(info.address);
    int paramCount = 0;

    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SVR, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open sovranocoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SovranoCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SovranoCoin.lnk
...
{
    return GetAutostartDir() / "sovranocoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a sovranocoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SovranoCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the sovranocoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add sovranocoin app to startup item list
