                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSVR ought to be enough for anybody." - Bill Gates, 2017
    ui->zSVRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSVRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSVR->setEnabled(false);
        ui->pushButtonMintzSVR->setToolTip(tr("zSVR is currently disabled due to maintenance."));

        ui->pushButtonSpendzSVR->setEnabled(false);
        ui->pushButtonSpendzSVR->setToolTip(tr("zSVR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSVRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSVR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSVR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSVR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSVR in ") +
...
void PrivacyDialog::on_pushButtonSpendzSVR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSVR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSVR
        sendzSVR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSVR
    sendzSVR();
}

void PrivacyDialog::on_pushButtonZSVRControl_clicked()
{
    ZSVRControlDialog* zSVRControl = new ZSVRControlDialog(this);
    zSVRControl->setModel(walletModel);
    zSVRControl->exec();
}

void PrivacyDialog::setZSVRControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSVRSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSVR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SovranoCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSVRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSVR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SVR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSVR</b>";
    ...
    // use mints from zSVR selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSVRControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSVRControlDialog::GetSelectedMints();
    }
    ...
    // Clear zSVR selector in case it was used
    ZSVRControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSVR Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SVR, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSVR Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSVR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSVR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSVR "));
    ui->labelzSVRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSVRSyncStatus->setVisible(fShow);
}
