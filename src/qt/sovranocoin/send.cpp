...
#include "qt/sovranocoin/send.h"
#include "qt/sovranocoin/forms/ui_send.h"
#include "qt/sovranocoin/addnewcontactdialog.h"
#include "qt/sovranocoin/qtutils.h"
#include "qt/sovranocoin/sendchangeaddressdialog.h"
#include "qt/sovranocoin/optionbutton.h"
#include "qt/sovranocoin/sendconfirmdialog.h"
#include "qt/sovranocoin/myaddressrow.h"
#include "qt/sovranocoin/guitransactionsutils.h"
...
SendWidget::SendWidget(SOVRANOCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-svr");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onSVRSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onSVRSelected(false);});
...
void SendWidget::refreshAmounts()
{
    ...
    bool isZsvr = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZsvr));
    ...
    if (CoinControlDialog::coinControl->HasSelected()) {
        ...
        totalAmount = (isZsvr ?
                ...
                walletModel->getUnlockedBalance(nullptr, fDelegationsChecked)) - total;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, isZsvr))
        );
}
...
void SendWidget::onSendClicked()
{
    ...
    bool sendSvr = ui->pushLeft->isChecked();
    ...
    if ((sendSvr) ? send(recipients) : sendZsvr(recipients)) {
...
bool SendWidget::sendZsvr(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if (sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zSVR is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (!ZSvrControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZSvrControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZsvr(
        ...
        inform(tr("zSVR transaction sent!"));
        ZSvrControlDialog::setSelectedMints.clear();
        clearAll(false);
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZSVR_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zSVR require a security level of 100 to successfully spend.");
        } else {
        ...
        Q_EMIT message("zSVR transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isSVR) {
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zSvrControl->exec();
            ui->btnCoinControl->setActive(!ZSvrControlDialog::setSelectedMints.empty());
            zSvrControl->deleteLater();
        } else {
            inform(tr("You don't have any zSVR in your balance to select."));
        }
    }
}
...
void SendWidget::onSVRSelected(bool _isSVR)
{
    isSVR = _isSVR;
    setCssProperty(coinIcon, _isSVR ? "coin-icon-svr" : "coin-icon-zsvr");
    refreshView();
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination svrAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(svrAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(svrAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
