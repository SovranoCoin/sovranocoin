...
#include "qt/sovranocoin/forms/ui_addresseswidget.h"
#include "qt/sovranocoin/addresslabelrow.h"
#include "qt/sovranocoin/addnewaddressdialog.h"
#include "qt/sovranocoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(SOVRANOCOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination svrAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(svrAdd, label.toUtf8().constData(),
