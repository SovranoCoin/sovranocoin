...
#include "qt/sovranocoin/topbar.h"
#include "qt/sovranocoin/forms/ui_topbar.h"
#include "qt/sovranocoin/lockunlock.h"
#include "qt/sovranocoin/qtutils.h"
#include "qt/sovranocoin/receivedialog.h"
#include "qt/sovranocoin/loadingdialog.h"
...
TopBar::TopBar(SOVRANOCOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezSvr, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzSvr, ui->labelTitleImmaturezSvr};
    ...
    setCssProperty({ui->labelAmountTopSvr, ui->labelAmountTopzSvr}, "amount-small-topbar");
    setCssProperty({ui->labelAmountSvr, ui->labelAvailablezSvr}, "amount-topbar");
    setCssProperty({ui->labelPendingSvr, ui->labelPendingzSvr, ui->labelImmatureSvr, ui->labelImmaturezSvr}, "amount-small-topbar");
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // PIV Total
    CAmount svrAvailableBalance = balance;
    // zPIV Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalSvr = GUIUtil::formatBalance(svrAvailableBalance, nDisplayUnit);
    QString totalzSvr = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // PIV
    // Top
    ui->labelAmountTopSvr->setText(totalSvr);
    // Expanded
    ui->labelAmountSvr->setText(totalSvr);
    ui->labelPendingSvr->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureSvr->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ...
    ui->labelAmountTopzSvr->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzSvr->setText(totalzSvr);
    // Expanded
    ui->labelAvailablezSvr->setText(totalzSvr);
    ui->labelPendingzSvr->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezSvr->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
