...
#include "qt/sovranocoin/forms/ui_snackbar.h"
#include "qt/sovranocoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SOVRANOCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SOVRANOCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
