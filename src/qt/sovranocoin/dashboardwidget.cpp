...
#include "qt/sovranocoin/dashboardwidget.h"
#include "qt/sovranocoin/forms/ui_dashboardwidget.h"
#include "qt/sovranocoin/sendconfirmdialog.h"
#include "qt/sovranocoin/txrow.h"
...
DashboardWidget::DashboardWidget(SOVRANOCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareSvr, "square-chart-svr");
    setCssProperty(ui->labelSquarezSvr, "square-chart-zsvr");
    setCssProperty(ui->labelSvr, "text-chart-svr");
    setCssProperty(ui->labelZsvr, "text-chart-zsvr");
    ...
    setCssProperty(ui->labelAmountSvr, "text-stake-svr-disable");
    setCssProperty(ui->labelAmountZsvr, "text-stake-zsvr-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &SOVRANOCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &SOVRANOCOINGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &SOVRANOCOINGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isSvr = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZSVR;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            svr = (pair.first != 0) ? pair.first / 100000000 : 0;
            zsvr = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalSvr += pair.first;
            chartData->totalZsvr += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSvr.append(svr);
        chartData->valueszSvr.append(zsvr);

        int max = std::max(svr, zsvr);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesSvr);
    set1->append(chartData->valueszSvr);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalSvr > 0 || chartData->totalZsvr > 0) {
        setCssProperty(ui->labelAmountSvr, "text-stake-svr");
        setCssProperty(ui->labelAmountZsvr, "text-stake-zsvr");
    } else {
        setCssProperty(ui->labelAmountSvr, "text-stake-svr-disable");
        setCssProperty(ui->labelAmountZsvr, "text-stake-zsvr-disable");
    }
    forceUpdateStyle({ui->labelAmountSvr, ui->labelAmountZsvr});
    ui->labelAmountSvr->setText(GUIUtil::formatBalance(chartData->totalSvr, nDisplayUnit));
    ui->labelAmountZsvr->setText(GUIUtil::formatBalance(chartData->totalZsvr, nDisplayUnit, true));
