...
#include "qt/sovranocoin/splash.h"
#include "qt/sovranocoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("sovranocoin-core", psz).toStdString();
}
...
{
    window = new SOVRANOCOINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &SOVRANOCOINGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &SOVRANOCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(SOVRANOCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SOVRANOCOINGUI::DEFAULT_WALLET);
        ...
        connect(window, &SOVRANOCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. SOVRANOCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(sovranocoin_locale);
    Q_INIT_RESOURCE(sovranocoin);
    ...
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("SOVRANOCOIN Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("SOVRANOCOIN Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("SOVRANOCOIN Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
