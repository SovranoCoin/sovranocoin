...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,           (numeric) the total sovranocoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zSVRsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zSVR denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zSVR denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zSVR denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zSVR denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zSVR denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zSVR denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zSVR denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zSVR denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zSVR denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,      (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,          (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,        (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,           (numeric) the transaction fee set in sovranocoin/kb\n"
            "  \"relayfee\": x.xxxx,           (numeric) minimum relay fee for non-free transactions in sovranocoin/kb\n"
    ...
    UniValue zsvrObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        if (mapZerocoinSupply.empty())
            zsvrObj.push_back(Pair(std::to_string(denom), ValueFromAmount(0)));
        else
            zsvrObj.push_back(Pair(std::to_string(denom), ValueFromAmount(mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zsvrObj.push_back(Pair("total", ValueFromAmount(GetZerocoinSupply())));
    obj.push_back(Pair("zSVRsupply", zsvrObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"sovranocoinaddress\"\n"
            "\nReturn information about the given sovranocoin address.\n"

            "\nArguments:\n"
            "1. \"sovranocoinaddress\"     (string, required) The sovranocoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"sovranocoinaddress\",    (string) The sovranocoin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for SOVRANOCOIN cold staking\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are sovranocoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) sovranocoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"sovranocoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"sovranocoinaddress\"  (string, required) The sovranocoin address to use for the signature.\n"
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in sovranocoin.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in sovranocoin.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) SVR value of the stakeable coins (minus reserve balance, if any)\n"
