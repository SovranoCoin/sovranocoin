...
#include "zsvr/zsvrwallet.h"


CzSVRTracker::CzSVRTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzSVRTracker::~CzSVRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSVRTracker::Init()
{
...
bool CzSVRTracker::Archive(CMintMeta& meta)
{
...
bool CzSVRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSVRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSVRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSVRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSVRTracker::GetSerialHashes()
{
...
CAmount CzSVRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSVRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSVRTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSVRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSVRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSVRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSVRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSVRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSVRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSVRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzSVRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSVRWallet* zSVRWallet)
{
    bool iszSVRWalletInitialized = (nullptr != zSVRWallet);
    CMintMeta meta;
    ...
    if (!iszSVRWalletInitialized)
        zSVRWallet = new CzSVRWallet(wallet);
    meta.isSeedCorrect = zSVRWallet->CheckSeed(dMint);
    if (!iszSVRWalletInitialized)
        delete zSVRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSVRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSVRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSVRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSVRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSVRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSVRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzsvr from DB\n", __func__, listDeterministicDB.size());
    }
