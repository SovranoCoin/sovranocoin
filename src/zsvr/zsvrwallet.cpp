...
CzSVRWallet::CzSVRWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSVRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSVRSeed_deprecated()) {
                    LogPrintf("%s: Updated zSVR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsvr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsvr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSVRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSVRCount(nCountLastUsed);
    else if (!walletdb.ReadZSVRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSVRWallet::Lock()
{
    seedMaster.SetNull();
}

void CzSVRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSVRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSVR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSVRWallet::LoadMintPoolFromDB()
{
...
void CzSVRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSVRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSVRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zsvrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSVRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSVR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    wallet->zsvrTracker->Add(dMint, true);
...
void CzSVRWallet::SeedToZSVR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzSVRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSVRWallet::GenerateDeterministicZSVR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSVRWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSVRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSVRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
