...
bool CLegacyZSvrStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZSvrStake from non zc-spend", __func__);
    ...
    *this = CLegacyZSvrStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zsvr stake origin", __func__);

    // All good
    return true;
}

CLegacyZSvrStake::CLegacyZSvrStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZSvrStake::GetIndexFrom()
{
...
CAmount CLegacyZSvrStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZSvrStake::GetUniqueness() const
{
...
bool CLegacyZSvrStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zSVR stake block: height %d outside range", __func__, nHeight);
