...
#if defined(NDEBUG)
#error "SOVRANOCOIN cannot be compiled without assertions."
#endif
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zSVR spend tx %s already in block %d", __func__, tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ZSVRModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                    return false;
                }
            ...
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zSVR mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < 20001 && nHeight > 0) {
        nSubsidy = 0.7 * COIN;
    } else if (nHeight < 40001 && nHeight >= 20001) {
        nSubsidy = 0.9 * COIN;
    } else if (nHeight < 60001 && nHeight >= 40001) {
        nSubsidy = 1.1 * COIN;
    } else if (nHeight < 80001 && nHeight >= 60001) {
        nSubsidy = 1.3 * COIN;
    } else if (nHeight < 100001 && nHeight >= 80001) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight < 120001 && nHeight >= 100001) {
        nSubsidy = 1.7 * COIN;
    } else if (nHeight < 140001 && nHeight >= 120001) {
        nSubsidy = 1.9 * COIN;
    } else if (nHeight < 160001 && nHeight >= 140001) {
        nSubsidy = 2.1 * COIN;
    } else if (nHeight < 180001 && nHeight >= 160001) {
        nSubsidy = 2.3 * COIN;
    } else if (nHeight < 200001 && nHeight >= 180001) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight < 250001 && nHeight >= 200001) {
        nSubsidy = 2.3 * COIN;
    } else if (nHeight < 300001 && nHeight >= 250001) {
        nSubsidy = 1.9 * COIN;
    } else if (nHeight < 350001 && nHeight >= 300001) {
        nSubsidy = 1.7 * COIN;
    } else if (nHeight < 400001 && nHeight >= 350001) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight < 450001 && nHeight >= 400001) {
        nSubsidy = 1.3 * COIN;
    } else if (nHeight < 500001 && nHeight >= 450001) {
        nSubsidy = 1.1 * COIN;
    } else if (nHeight < 600001 && nHeight >= 500001) {
        nSubsidy = 0.9 * COIN;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZSVRModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    if (!UpdateZSVRSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zSVR supply", __func__);
...
{
    util::ThreadRename("sovranocoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZSVRSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zSVR supply for block=%s height=%d", __func__,
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateSVRSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_ZC_RecalcAccumulators) {
        ...
        const CAmount nInvalidAmountFiltered = 2900*COIN;    //Amount of invalid coins filtered through exchanges, that should be considered valid
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSVR serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nSovranocoinBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nSovranocoinBadBlockBits)) {
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nVersion >= 5 && pindexPrev) { // pindexPrev is only null on the first block which is a version 1 block.
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> svrInputs;
        std::vector<CTxIn> zSVRInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zSVRInputs.push_back(stakeIn);
            }else{
                svrInputs.push_back(stakeIn);
            }
        }
        const bool hasSVRInputs = !svrInputs.empty();
        const bool hasZSVRInputs = !zSVRInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasSVRInputs) {
                    ...
                    for (const CTxIn& svrIn : svrInputs)
                        if(svrIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            if(hasZSVRInputs) {
                for (const CTxIn& zSvrInput : zSVRInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zSvrInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsvr");
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70919 (v3.0.0)
    //if (sporkManager.IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
    //        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 was used for 70916 (v3.1.0), commented out now.
