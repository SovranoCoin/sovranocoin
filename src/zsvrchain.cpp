...
{
    CDataStream serializedCoinSpend = ZSVRModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZSVRSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
...
bool UpdateZSVRSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
