...
bool CSvrStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CSvrStake from zerocoin spend", __func__);
...
bool CSvrStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CSvrStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CSvrStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CSvrStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CSvrStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CSvrStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CSvrStake::GetUniqueness() const
{
...
CBlockIndex* CSvrStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CSvrStake::ContextCheck(int nHeight, uint32_t nTime)
{
